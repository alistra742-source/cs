...
async def _start_real_demo_browser(wid: str, cfg: dict) -> dict:
    ...
    The QA browser intentionally uses a DIRECT connection.  It does not
    rotate proxies, use TOR, or borrow the Discord signup worker's
    transport: the Test/Live-Demo flow must work without depending on the
    proxy pool (dead sessions or an exhausted vaultproxies quota would
    otherwise break the test with a chrome-error page).
    """
    state = _workers.get(wid) or _init_worker(wid)
    _workers[wid] = state
    if state.get("status") in ("starting", "running"):
    ...
    try:
        ...
        if not alive:
            ...
            # A parked signup browser may have a proxy or TOR attached.
            # Rebuild it direct before using it for the public demo.
...
async def _vision_warmup(interval: float = 600.0) -> None:
    """Keep the Hugging Face serverless model warm.

    Serverless Inference unloads a model that gets no traffic, and the next
    captcha then pays a 20-60s cold start (HTTP 503 "model is loading")
    which usually expires the challenge. A tiny GET on the model route
    every 10 minutes keeps it resident and also reports auth/model
    misconfiguration early instead of mid-solve.
    """
    import vision_solver as _vs
    if not _vs.API_KEY:
        _log("[Vision] API_KEY is not set — the Hugging Face solver is "
             "disabled until it is configured", level="warn")
        return
    client = _vs.HFVisionClient(log=lambda m, level="info": _log(m, level=level))
    ...
    while True:
        ...
        if was_up is not None and up != was_up:
            if up:
                _log(f"[Vision] Hugging Face back UP ({client.model})",
                     level="warn")
            else:
                _log(f"[Vision] Hugging Face DOWN ({client.model}) — captcha "
...
def main() -> None:
    ...
    # Keep the Hugging Face serverless model warm (no-op without API_KEY).
    try:
