...
@app.route('/')
def handle_root():
    # Inject the LIVE CONTROL overlay (live_ui.LIVE_INJECTION) just before
    # the FINAL </body> so the dashboard actually gets the LIVE tab — it was
    # being built in live_ui.py but never attached, so opening LIVE showed
    # nothing and there was no way back to the main page. Only the last
    # closing tag is targeted: the viewAllLogs() helper also embeds a
    # "</body></html>" string, and a naive replace() would corrupt it.
    html = DASHBOARD_HTML
    idx = html.rfind("</body>")
    if idx != -1:
